...
def dilate(data):
    ...
    for i in range(data.shape[2]):
        dilate_data = cv2.dilate(data[:,:,i], kernel)
...
#定义OTSU算法的膨胀和腐蚀优化函数
def dilate_threshold(data):
    # band = data.GetRasterBand(2).ReadAsArray(0, 0, data.shape[0], data.shape[1]) #提取绿波段
    # band_array = band.astype(np.uint8)
    band_array=data[:,:,1]
    threshold, binary = cv2.threshold(band_array, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
    result = cv2.dilate(binary, kernel)
    return result

def erode_threshold(data):
...
def open(img):
    # img=erode_threshold(img)
    img=erode(img)
    # print(img.shape,img.ndim)
    # img=dilate_threshold(img)
    img=dilate(img)
    return img
...
def gauss_division(image):
    '''高斯滤波，图像除法，原理同单尺度Retinex算法
    即通过大核模糊提取光照信息，从原图像中去除
    image cv2'''
...
# images/e66.jpg
# scanner/binary/imgs/wenzi2.jpg
img=cv2.imread('../images/e69.jpg')
# cv2.imshow("img", img)
# cv2.waitKey()
pic=1   # 分块
h,w=img.shape[:2]
...
cv2.waitKey()
cv2.destroyAllWindows()
cv2.imwrite('D:/file/class/cv/code/scanner/binary/imgs/e69.jpg', img)